...
<section class="content">
  ...
  <div class="card">
    {!! Form::model($knowledgeBlog, ['method' => 'PATCH','route' => ['knowledge-blogs.update', $knowledgeBlog->id]]) !!}
        ...
        <div class="card-body">
                ...
                <div class="col-12">
                    <div class="form-group">
                        ...
                        <div class="input-group">
                            ...
                            <input id="blog_attachment" class="form-control" type="text" name="blog_attachment" value="{{$knowledgeBlog->blog_attachment}}">
                        </div>
                    </div>
                </div>
            </div>
            {{-- <div class="row mb-3">
                <div class="col-12 files-wrapper">
                    @if(count($knowledgeBlog->attachments) > 0)
                        <input type="hidden" name="file_count" id="file_count" value="{{count($knowledgeBlog->attachments)}}">
                        @foreach($knowledgeBlog->attachments as $key=>$att)
                        <div class="form-group file-{{$key+1}}">
                            @if($key == 0)
                            ...
                            <div class="input-group">
                                <span class="input-group-btn">
                                <a data-input="attachment_{{$key+1}}" data-preview="holder" class="btn btn-primary text-white lfm">
                                <i class="far fa-image"></i> Choose
                                </a>
                                </span>
                                <input id="attachment_{{$key+1}}" class="form-control" type="text" name="attachment[]" value="{{$att->attachment}}">
                            </div>
                        </div>
                        @endforeach
                    @else
                ...
                <div class="col-12">
                    <div class="form-group">
                        ...
                        <div class="input-group">
                            ...
                            <input id="image" class="form-control" type="text" name="image" value="{{$knowledgeBlog->image}}">
                        </div>
                    </div>
                </div>
            </div>
                ...
                <div class="col-12">
                    ...
                    <div class="form-group">
                        <label for="video_upload" style="margin-right: 20px;font-weight:300;">
                            <input type="radio" name="video_upload_type" id="video_upload" class="upload-type" value="0" {{$knowledgeBlog->video_upload_type == "0" ? 'checked':''}}> Video Upload
                        </label>
                        <label for="youtube_upload" style="font-weight:300;">
                            <input type="radio" name="video_upload_type" id="youtube_upload" class="upload-type" value="1" {{$knowledgeBlog->video_upload_type == 1 ? 'checked':''}}> Youtube URL
                        </label>
                    </div>
                </div>
                ...
                <div class="col-12 video-type video d-none">
                    <div class="form-group">
                        ...
                        <div class="input-group">
                            ...
                            <input id="video" class="form-control" type="text" name="video" value="{{$knowledgeBlog->video}}">
                        </div>
                    </div>
                </div>
...
<script>
    $(function() {
        ...
        var upload_type = "{{$knowledgeBlog->video_upload_type}}";
    ...
    @if(count($knowledgeBlog->attachments) > 0)
        var countFile = {{count($knowledgeBlog->attachments)}};
    @else
        var countFile = 1;
    @endif
