...
class MessagesController extends Controller
{
    ...
    public function idFetchData(Request $request)
    {
        ...
        if ($request['type'] == 'user') {
            ...
            $fetch = ChatGroup::where('ref_no', $request['id'])->first();
        }
    ...
    public function send(Request $request)
    {
        ...
        if (!$error_msg) {
            if(isset($request['type'])) {
                ...
                $error_msg = "Your connection is not stable!";
            }
        }
    ...
    public function fetch(Request $request)
    {
        ...
        if($request['type'] == 'user') {
            ...
            $query = Chatify::fetchGroupMessagesQuery($request['id'])->orderBy('created_at', 'asc');
        }
        ...
        if ($query->count() > 0) {
            foreach ($messages as $message) {
                if($request['type'] == 'user') {
                    ...
                    $allMessages .= Chatify::messageCard(
                        Chatify::fetchGroupMessage($message->id)
                    );
                }
    ...
    public function seen(Request $request)
    {
        ...
        if($request['type'] == 'user') {
            ...
            $seen = Chatify::makeGroupSeen($request['id']);
        }
    ...
    public function getContacts(Request $request)
    {
        ...
        //     ->get()
        //     ->unique('id');
        $enrol_users = EnrolUser::where('course_id', $request->course_id)->pluck('user_id');

        $users = Message::join('users',  function ($join) {
            $join->on('ch_messages.from_id', '=', 'users.id')
                ->orOn('ch_messages.to_id', '=', 'users.id');
        })
        ->whereIn('users.id', $enrol_users)
        ...
        if(isset($request->course_id)) {
            $group = ChatGroup::where('course_id', $request->course_id)->first();
            $contacts .= Chatify::groupContactItem($request['messenger_id'], $group);
        }else {
            $authuser = User::find(Auth::id());
            if($authuser->hasRole(['Super Admin'])) {
                $enrol_courses = Course::latest('id')->pluck('id');
            }else {
                $enrol_courses = EnrolUser::where('user_id', Auth::id())->pluck('course_id');
            }
            
            $groups = ChatGroup::whereIn('course_id', $enrol_courses)->orderBy('id','desc')->get()->unique('id');
            if($groups->count() > 0) {
        ...
        //     $contacts .= Chatify::groupContactItem($request['messenger_id'], $group);
        // }else {
        //     $contacts .= Chatify::groupSearchItem($request['messenger_id'], $group);
        // }
    ...
    public function updateContactItem(Request $request)
    {
        ...
        if($request['type'] == 'user') {
            ...
            $group = ChatGroup::where('ref_no', $request['messenger_id'])->first();
            $contactItem = Chatify::groupContactItem($request['messenger_id'], $group);
        }
    ...
    public function search(Request $request)
    {
        ...
        if(isset($request->course_id)) {
            $enrol_users = EnrolUser::where('course_id', $request->course_id)->pluck('user_id');
            $records = User::where('name', 'LIKE', "%{$input}%")->whereIn('id', $enrol_users);
        }else {
