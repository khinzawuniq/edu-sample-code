...
class ChatifyMessenger
{
    ...
    public function fetchMessage($id){
        ...
        if(isset($msg->attachment) && !empty($msg->attachment)){
            // $attachmentOBJ = json_decode($msg->attachment);
            $attachmentOBJ = explode(',',$msg->attachment);
            $attachment = $attachmentOBJ[0];
            $attachment_title = $attachmentOBJ[1];
    ...
    public function fetchGroupMessage($id){
        $attachment = null;
        $attachment_type = null;
        $attachment_title = null;
    ...
    public function fetchGroupMessagesQuery($ref_no){
        return Message::where('to_id', '=', $ref_no)
                    ->orWhere('from_id', '=', $ref_no);
    }
    ...
    public function makeGroupSeen($ref_no){
        Message::Where('to_id',$ref_no)
                ->where('seen',0)
                ->update(['seen' => 1]);
        return 1;
    }
    ...
    public function getGroupLastMessageQuery($ref_no){
        return $this->fetchGroupMessagesQuery($ref_no)->latest()->first();
    }
    ...
    public function countGroupUnseenMessages($ref_no){
        return Message::where('to_id',$ref_no)->where('seen',0)->count();
    }
    ...
    public function groupContactItem($messenger_id, $group){

        $group_msg_count = Message::where('from_id', $group->ref_no)->orWhere('to_id', $group->ref_no)->count();
        if($group_msg_count > 0) {
            // get last message
            $lastMessage = $this->getGroupLastMessageQuery($group->ref_no);

            // Get Unseen messages counter
            $unseenCounter = $this->countGroupUnseenMessages($group->ref_no);
            // $unseenCounter = '';
        }else {
            // get last message
            $lastMessage = '';
    ...
    public function groupSearchItem($messenger_id, $group){
        ...
        if($msgs->count() > 0){
            foreach ($msgs->get() as $msg) {
                ...
                if($msg->attachment){
                    ...
                    $attachment = explode(',',$msg->attachment);
                    // determine the type of the attachment
                    in_array(pathinfo($attachment[0], PATHINFO_EXTENSION), $this->getAllowedImages())
        ...
        try {
            foreach ($this->fetchMessagesQuery($user_id)->get() as $msg) {
                ...
                if (isset($msg->attachment)) {
                    $attachment = explode(',',$msg->attachment);
                    $path = storage_path('app/public/'.config('chatify.attachments.folder').'/'.$attachment[0]);
